...
const dest = path.join(__dirname, '../../uploads/team');
...
function copyFile (source, filename) {
  fs.copyFile(source, path.join(__dirname,'../uploads/team', filename), (err) => {
      if (err) throw err;
...
exports.create = (req, res) => {
  ...
  team.save(function (err) {
    ...
    fs.copyFile(
      req.file.path,
      path.join(__dirname, '..', '..', 'uploads', 'team', req.file.filename),
      (err) => {
        if (err) throw err;
...
Team.findOne({name: name}, (err, item) => {
    if (err) {
...
exports.delete = (req, res) => {
  ...
  Team.findByIdAndRemove(id, (err, result) => {
    if (err) {
      res.status(500).json(err).end()
    }
    res.json(result)
...
exports.update = (req, res) => {
  console.log(req.body);
  if (req.file) {
    req.body.photo = '/uploads/team/' + req.file.filename
  }
  Team.findOneAndUpdate({_id: req.params.id}, req.body, function (err, data) {
    if (err) {
      return res.json(err)
    }
    if (req.file) {
...
exports.patch = async (req, res) => {
  ...
  console.log(filenames);
  req.body.images = filenames
  const doc = await Team.findOne({_id: req.params.id});
