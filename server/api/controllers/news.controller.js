...
const dest = path.join(__dirname, '../../uploads/news');
...
function copyFile (source, filename) {
  fs.copyFile(source, path.join(__dirname,'../uploads/news', filename), (err) => {
      if (err) throw err;
...
exports.create = (req, res) => {
  ...
  news.save(function (err) {
    ...
    fs.copyFile(
      req.file.path,
      path.join(__dirname, '..', '..', 'uploads', 'news', req.file.filename),
      (err) => {
        if (err) throw err;
        console.log('jajaja');
        fs.readdir(directory, (err, files) => {
...
News.findOne({vendor: vendor}, (err, item) => {
    if (err) {
...
exports.update = (req, res) => {
  console.log(req.body);
  if (req.file) {
    req.body.photo = '/uploads/news/' + req.file.filename
  }
  News.findOneAndUpdate({_id: req.params.id}, req.body, function (err, data) {
    if (err) {
      return res.json(err)
    }
    if (req.file) {
...
exports.patch = async (req, res) => {
  ...
  console.log(filenames);
  req.body.images = filenames
  const doc = await News.findOne({_id: req.params.id});
